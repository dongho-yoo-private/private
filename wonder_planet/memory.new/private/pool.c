...
#if MEMORY_DEBUG==1
const bit32_t magic_no=0x01234567F;
#endif
...
void* memory_pool_malloc(void* ptr, size_t n)
{
    ...
    if (pool_header->debug_count==4507)
    {
        while(0);
    }
#endif
    __MUTEX_UNLOCK__
    printf("memory_pool_malloc exit.......................(%d,%p)\n",pool_header->debug_count, (void*)(block+1));
    if (pool_header->debug_count==842)
    {
        while(0);
    }
    return (void*)(block+1);
