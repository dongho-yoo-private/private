...
Bool yiSmtpSendEx(HandleSmtp hSmtp, Bool bSendThread, HWND hNotifyWnd, UINT msg)
#else
// -------------------------------------------
EXTERN Bool yiSmtpSendEx(HandleSmtp hSmtp, Bool bSendThread);
#endif
{
	sSmtpDetail* s = (sSmtpDetail*)hSmtp;
	sockssl_t sock=0;
	...
	if (s->id)
	{
		...
		//printf(AUTH_LOGIN);
		if (yisend_ssl(sock, AUTH_LOGIN, AUTH_LOGIN_LEN, False)!=AUTH_LOGIN_LEN)
		{
			retCode=2;
			goto SEND_FAILURE;
		}
		...
		sprintf(x, "%s\r\n", pBase64);
		if (yisend_ssl(sock, x, strlen(x), False)!=strlen(x))
		{
			retCode=2;
			goto SEND_FAILURE;
		}
...
SEND:
	...
	else
	{
		...
		if (stricmp(s->dataencoding, "base64")==0)
		{
			int n;
			pBase64 = yiToBase64(s->p, s->size, &n);
		...
		yimListAdd(hBodyStr, "\r\n");
		yimListAdd(hBodyStr, "Content-Type: ");
		yimListAdd(hBodyStr, s->dataType);
		yimListAdd(hBodyStr, ";\r\n\tname=\"");
		yimListAdd(hBodyStr, s->filename);
		yimListAdd(hBodyStr, "\"\r\n");
		yimListAdd(hBodyStr, "Content-Transfer-Encoding: ");
		yimListAdd(hBodyStr, s->dataencoding);
		yimListAdd(hBodyStr, "\r\n");
		...
		{
			if (yisend_ssl(sock, s->p, s->size)!=s->size)
			{
				retCode=6;
				goto SEND_FAILURE;
			}
		...
		// 最後に。。。
		{
			...
			sprintf(buffer, "\r\n--%s--\r\n\r\n.\r\n", szBoundary);
			if (yisend_ssl(sock, buffer, (len=strlen(buffer)))!=len)
			{
				retCode=6;
				goto SEND_FAILURE;
			}
