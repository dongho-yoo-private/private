...
char** cyiGoEngine::CreateMap(cyiTreeNode* node)
{
	...
	while(stack.pop(val)==True)
	{
		...
		{
			for (cyiNode* node=info->m_AddStoneList->GetNodeTopFrom(); node; node->np)
			{
...
Bool cyiGoEngine::MakeTitle(char* title)
{
	...
	{
		wsprintf(title, "%s VS %s", m_header->pszBlackName, m_header->pszWhiteName);
		return True;
	}
...
Bool cyiGoEngine::Create(sGoHeader* header/*=0*/, sGobanContentsDetail* detail/*=0*/)//eGoNodeType eType/*=eGNT_NORMAL*/, eCharacterSet eLocale/*=eCS_JAPANESE*/)
{
	...
	{
		char buffer[256];
		wsprintf(buffer, "%s VS %s", header->pszBlackName, header->pszWhiteName);
...
Bool cyiGoEngine::Create(const void* pBlock, int size, sGobanContentsDetail* detail)
{
	...
	int nInitStone;
	Bool bIsRepair=detail->eContentsMode==eGCM_JOSEKI_EDITOR?True:False;
...
Bool cyiGoEngine::Create(const char* pszFileName, sGobanContentsDetail* detail)//eGoFileType format/*=eGF_SGF*/, eGoNodeType eType/*=eGNT_NORMAL*/, eCharacterSet eLocale/*=eCS_JAPANESE*/)
{
	...
	{
		if (m_sgfMgr.LoadSgfFile(pszFileName, detail->eContentsMode==eGCM_JOSEKI_EDITOR?True:False)==False)
		{
			return False;
		}
	}
	...
	{
		if (m_sgfMgr.LoadBinaryGoFormat(pszFileName, 0, 0)==False)
		{
			return False;
		}
	}
...
stone_t** cyiGoEngine::PutStone(stone_t stone, aStoneList branches, Bool bIsAdded/*=False*/, Bool bIsPlayMode/*=False*/)
{
	...
	int n = m_sgfMgr.Add(stone, nCatchedStone, bIsAdded);
...
{
	if (type==eGFT_SGF)
		return m_sgfMgr.SaveSgfFile(pszFileName);
...
const char* cyiGoEngine::GetTitle()
{
	...
	{
		wsprintf(m_szTitle, "%s VS %s", m_sgfMgr.m_header->pszBlackName, m_sgfMgr.m_header->pszWhiteName);
		return m_szTitle;
	}
	if (m_szFileName[0])
	{
	...
	//return 0;
	wsprintf(m_szTitle, "名前なし-%d", GetGoId());
...
cyiList* cyiGoEngine::GetLabel(Bool* bIsReqFree)
{
	...
	if (xGET_GOBAN_MODE(m_flag)!=eGCM_JOSEKI_EDITOR)
	{
		if (m_parent==0 ||
				xGET_GOBAN_MODE(m_parent->m_flag)!=eGCM_JOSEKI_EDITOR)
		{
