...
size_t ADK_ResetAllocCount() {
  gAllocCount = 0;
  return gAllocCount;
}
size_t ADK_GetAllocCount() { return gAllocCount; }
...
void operator delete(void* p) throw() { ADK_Free(p); }
void operator delete[](void* p) throw() { ADK_Free(p); }
...
void operator delete(void* p, const std::nothrow_t&) throw() { ADK_Free(p); }
void operator delete[](void* p, const std::nothrow_t&) throw() { ADK_Free(p); }
