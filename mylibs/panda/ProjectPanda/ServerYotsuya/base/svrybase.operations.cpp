...
Bool csvrybase::CreditWrite(const char* pszFileName, const void* buffer, int size, int check_sum_interval)
{
	...
	//sThreadParam s = {this, pDatas, 0};
	if (yiThreadStartEx(this, &csvrybase::OnCreditWrite, pDatas)== (pthread_t)-1)
	{
		return False;
	}
	return True;
...
Bool csvrybase::CreditAppendRecord(const char* pszFileName, const void* buffer, int size, int check_sum_interval)
{
	...
	if (yiThreadStartEx(this, &csvrybase::OnCreditAppendRecord, pDatas)== (pthread_t)-1)
	{
		return False;
	}
	return True;
