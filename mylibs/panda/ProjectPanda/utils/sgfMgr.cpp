...
#else
Bool cSgfManager::WriteSgfBody(int hFile, cyiTreeNode* t, Bool xxxFlag)
#endif
{
...
#else
		{
			int xlen = strlen(exbuffer);
			if (write(hFile, exbuffer, xlen)!=xlen)
			{
				return False;
			}
		}
...
#else
		{
		int xlen = strlen(buff);
		if (write(hFile, buff, xlen)!=xlen)
		{
			return False;
		}
		}
...
#else
	{
		int xlen=strlen(branch_end);
		if (write(hFile, branch_end, xlen)!=xlen)//, &nWritten, 0)==False)
		{
			return False;
		}
	}
...
#else
	{
		int xlen = strlen(buffer);
		if (write(hFile, buffer, xlen)!=xlen)//strlen(buffer), &nWritten, 0)==False)
		{
			return False;
		}
	}
...
#ifdef _xDEBUG
		{
			...
			WriteFile(hFile, buffer, strlen(buffer), &nWritten, 0);
#else
			write(hFile, buffer, strlen(buffer));//, &nWritten, 0);
#endif
		}
#endif	
		if (WriteSgfBody(hFile, ct, xxxFlag)==False)
...
Bool cSgfManager::SaveSgfFile(LPCSTR pszFileName, Bool xxxFlag)
{
...
#else
	int hFile = open(pszFileName, O_WRONLY, MODE_666);//GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
#endif
#ifdef _WIN32
...
#else
	if (write(hFile, szHeader, nHeaderLen)!=nHeaderLen)
#endif
	{
#ifdef _WIN32
		CloseHandle(hFile);
...
#else
	{
		int len = strlen(m_header->pszComment);
		if (write(hFile, m_header->pszComment, len)!=len)
		{
			close(hFile);
			return False;
		}
	...
	CloseHandle(hFile);
#else
	ftruncate(hFile, lseek(hFile, 0, SEEK_CUR));
	close(hFile);
#endif
...
//NEW_NODE:
	...
	currentInfo=(cSgfStoneInfo*)m_currentNode->GetData();
	node = m_currentNode;//new cyiTreeNode((yiAnyValue)currentInfo);
	ret=1;
