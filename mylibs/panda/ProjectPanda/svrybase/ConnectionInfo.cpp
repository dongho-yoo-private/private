...
// ------------------------------------
cConnectionInfo::cConnectionInfo(int n, int index, int acceptFd, Bool bIsLocal)
{
	yiTrace("cConnectionInfo::cConnectionInfo() in\n");
	int pipefd[2];
	...
	m_bIsLocal=bIsLocal;
	m_AcceptFd=acceptFd;
	yiTrace("cConnectionInfo::cConnectionInfo() out\n");
}
...
// ------------------------------------
Bool cConnectionInfo::Add(int fd)
{
//yiTrace("in add...................\n");
...
// ------------------------------------
int cConnectionInfo::AddFromQueue(OnAcceptedCallback callback, void* pClassCallback)
{
	yiMutexLock(m_hMutex);
...
// ------------------------------------
void cConnectionInfo::RemoveFd(int n)
{
	yiMutexLock(m_hMutex);
...
//
Bool cConnectionInfo::IsFull()
{
	return (m_cnt==m_maxcnt);
}

//
int* cConnectionInfo::GetFds()
{
	return m_fds;
}

//
void** cConnectionInfo::GetExtraDataPtr(int index)
{
	return &m_fdsExInfo[index];
}

//
void cConnectionInfo::FdSets()
{
...
//
Bool cConnectionInfo::Wait(cyiQueue& rdFds, OnAcceptedCallback callback, OnClosedCallback callback_close, void* pClassCallback)
{
	int res;
...
// ------------------------------------
void cConnectionInfo::UpdateMaxValue()
{
...
// ------------------------------------
void cConnectionInfo::BroadCastMessage(void* encoded_packet, int size)
{
	for (int i=0; i<m_maxcnt; i++)
	{
		if (m_fds[i]==-1)
...
// ------------------------------------
void cConnectionInfo::RequestTerminate(int reason)
{
...
// ------------------------------------
void cConnectionInfo::RemoveAll()
{
	for (int i=0; i<m_maxcnt; i++)
	{
		if (m_fds[i]!=-1)
