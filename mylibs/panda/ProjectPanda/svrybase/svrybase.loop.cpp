...
void csvrybase::OnLoopNormal(unsigned long param1, unsigned long param2, unsigned long param3)
{
	...
	//cConnectionManager& mgr = *(new cConnectionManager());
	cConnectionManager mgr; //= *(new cConnectionManager());
	...
	SYSLOG(LogLevel::NOTIFY, "OnLoopNormal %s listen success\n", bIsLocalLoop?"Local":"Server");


	if (pipe(pipe_fd)!=0)
	{
		SYSLOG(LogLevel::ERROR, "pipe error!\n");
		assert(0);
	}
	...
	{
		SYSLOG(LogLevel::ERROR, "unexpected case!\n");
		return ;
	}
	...
	while(bIsTerminate==False)
	{
		...
		pConnInfo = new cConnectionInfo(pInfo->nMaxConnections, mgr.GetArrayCnt(), pipe_fd[1], bIsLocalLoop);
		pConnInfo->Add(sock);
		mgr.Add(pConnInfo);

		pthread_t id = yiThreadStartExj(this, &csvrybase::OnDispatch, pConnInfo, bIsLocalLoop, pipe_fd[1]);


		if (id==-1)
		{
			SYSLOG(LogLevel::ERROR, "Thread Create Failure!!\n");
			m_bTerminate=True;
			break;
		}
	...
	yiTrace("csvrybase::OnLoopNormal() exit\n");
...
void csvrybase::OnLoopLimited(unsigned long param1, unsigned long param2, unsigned long param3)
{
	...
	SYSLOG(LogLevel::NOTIFY, "OnLoopLimited %s listen success\n", bIsLocalLoop?"Local":"Server");
	if (pipe(pipe_fd)!=0)
	{
		SYSLOG(LogLevel::ERROR, "pipe error!\n");
		assert(0);
	...
	yiTrace("csvrybase::OnLoopLimited() exit\n");
}
