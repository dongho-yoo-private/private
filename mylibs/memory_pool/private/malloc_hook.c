...
static struct {
    ...
    void (*free)(void*); 
    memory_free_handler_t free_handler;
    memory_allocation_handler_t allocate_handler;
} memory_hook_fncs={0,};

void init_memory_hook(memory_allocation_handler_t handler,memory_free_handler_t free_handler)
{
...
void* calloc(size_t n, size_t size)
{
    if (memory_hook_fncs.allocate_handler!=0)
    {
        const size_t all=n*size;
        return memset(memory_hook_fncs.allocate_handler(all),0,all);
    }
...
void* realloc(void* data, size_t n)
{
    ...
    {
        /* TODO どうする？　*/
        void* ret=data==0?memory_hook_fncs.allocate_handler(n):memcpy(memory_hook_fncs.allocate_handler(n), data, n);
        if (data!=0)
        {
             free(data);   
        }
