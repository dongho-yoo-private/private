...
void memory_pool_init_struct(void* ptr,size_t size,size_t align)
{
    ...
    pool_header->large_block_size=top->size;
    pool_header->large_block=top;
...
void memory_pool_print(void* ptr, const char* filters[], bit32_t sequence_no)
{
    ...
    int total_used_block_count=0,total_unused_block_count=0;
    size_t total_used_memory_size=0,remain_memory_sized=0;
    ...
    for (;p!=0;p=p->next)
    {
        ...
        {
            remain_memory_sized+=p->size;
            ++total_unused_block_count;
            MEM_DEBUG_PRINT(" 0x%p > %s unused\n", p+1,get_bytes(temp,p->size));
        }
    }
    MEM_DEBUG_PRINT("total used block count :%d, unused count: %d\n",total_used_block_count, total_unused_block_count );
    MEM_DEBUG_PRINT("used:%s, remain: %s\n",get_bytes(temp, total_used_memory_size), get_bytes(temp2, remain_memory_sized));
}
...
static memory_header_t* find_unused_block(memory_pool_header_t* pool_header, size_t n)
{
    ...
    if (pool_header->large_block!=0&&pool_header->large_block->size>=n)
    {
        return pool_header->large_block;
    }
...
static void split_block(memory_pool_header_t* memory_pool_header, memory_header_t* header, size_t n, size_t align)
{
    ...
    if (diff>0)
    {
        ...
        if (diff-align_diff>0)
        {
            ...
            if (memory_pool_header->large_block==header)
            {
                memory_pool_header->large_block=next;
                memory_pool_header->large_block_size=next->size;
            }
            return ;
        }
    }
    /* この場合は分割できないので、ヘッダーのサイズをそのままにしておく */
    if (header==memory_pool_header->large_block)
    {
        memory_pool_header->large_block_size=0;
    }
}
static void free_block(memory_pool_header_t* pool_header, memory_header_t* block)
{
    ...
    if (pool_header->large_block_size<header->size)
    {
        /* 空き領域を探す効率を高めるロジック 
         解放して、メモリ領域が大きくなれば、現在最も大きなメモリ領域を指すようにしておく */
        pool_header->large_block_size=header->size;
        pool_header->large_block=header;
    }
#if MEMORY_DEBUG==1
