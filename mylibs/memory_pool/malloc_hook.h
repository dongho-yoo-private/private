...
typedef void* (*memory_allocation_handler_t)(size_t size);
typedef void (*memory_free_handler_t)(void* data);
/**
* \brief malloc系のシステムコールをフックします。
* \param handler malloc系が呼び出された際に呼び出されるコールバック
* \param free_handler freeが呼び出された際に呼び出されるコールバック
*/
EXTERN_C void init_memory_hook(memory_allocation_handler_t handler,memory_free_handler_t free_handler);
/**
* \brief libcのmalloc
*/
