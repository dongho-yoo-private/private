...
public:
	...
	{
		stone=info->stone;
		label=info->label?new cyiList(info->label):0;
...
public:
	...
	// -----------------------------------
	Bool WriteSgfBody(HANDLE hFile, cyiTreeNode* t);

	// -----------------------------------
	Bool SaveSgfFile(LPCSTR pszFileName);

	// -----------------------------------
	Bool WriteBinaryGoFormat(cyiMemoryStream* stream, cyiTreeNode* t);
