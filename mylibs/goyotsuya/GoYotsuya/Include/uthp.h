...
// ------------------------------------
_EXTERN BOOL uthpSend(socket_t sock, int id, const void* buffer, int size, Bool bIsConfirm=True);
...
// ------------------------------------
_EXTERN int uthpTcpLogin(socket_t sock, unsigned int& id, const char* pszName, const char* pszPassword, sNetworkInfo* network, unsigned int* udpPort=0);//, socket_t acceptSock, void* buffer, int*receved);

// ------------------------------------
_EXTERN BOOL uthpSendRCU(socket_t tcpBloker, int myIndex, int remoteIndex, const char* pszPassword);

// ------------------------------------
_EXTERN BOOL uthpSendRCUx(socket_t sock, unsigned int remoteId);
...
// ------------------------------------
_EXTERN socket_t uthpUdpHolePunching2(int sockUdp, address_t remoteAddr, int port, int nRandNo, int nRetry);
