...
AW:
			...
			case 5: // C[]
			{
				...
				info->comment =xstrtoheap(&pIndex[1], ']');
...
Bool cSgfManager::LoadSgfFile(LPCSTR pszFileName, Bool bIsRepair, int locale)
{
	...
	strcpy(m_szFileName, pszFileName);
	return ImportBlock(m_sgfRaw, m_blocksize);
...
// -----------------------------------
// 
Bool cSgfManager::WriteSgfBody(HANDLE hFile, cyiTreeNode* t)
{
...
SKIP_STONE:
	...
	else if (nChildCnt==1)
	{
		...
		if (WriteSgfBody(hFile, ct)==False)
		{
			return False;
		}
		return True;
...
Bool cSgfManager::SaveSgfFile(LPCSTR pszFileName)
{
	...
	if (WriteSgfBody(hFile, &m_node)==False)
	{
		CloseHandle(hFile);
		return False;
	}
...
//32: xSV[
//34: xPD[ // 再生済み　定石辞典でしか使われない。
Bool cSgfManager::WriteBinaryGoFormat(cyiMemoryStream* stream, cyiTreeNode* t)
{
	int   nStartPos ;
	int	  nCommentLen=0;
	int	  nChildCnt=0;
...
#endif
			if (WriteBinaryGoFormat(stream, x)==False)
			{
				return False;
			}
...
SKIP_STONE:
	...
	else if (nChildCnt==1)
	{
		...
		if (WriteBinaryGoFormat(stream, ct)==False)
		{
			return False;
		}
		return True;
...
Bool cSgfManager::SaveBinaryGoFormat(LPCSTR pszFileName, void* header, int nHeaderSize)
{
	...
	if (WriteBinaryGoFormat(stream, &m_node)==True)
	{
		chunk=0;
		stream->Write(&chunk, 1, 0);
	...
	while(1)
	{
		...
		switch(id)
		{
			...
			case eYCID_COMMENT:
			{
				...
				char* pszComment=(char*)yialloci(len+1);
